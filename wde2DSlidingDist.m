...
%   Adrian M. Peter
%--------------------------------------------------------------------------
function slidingDist = wde2DSlidingDist(coeffs1, coeffs2, wName, startLevel, ...
...
% decompositions.
[distMats basesLocs] = euclidDistsBetwCoeffs(wName, startLevel,...
...
%     coeffOrder1 = linearAssignment(costMatrix, distMats{j}, lambda, addWeights, maximize);
    coeffOrder1 = linearAssignment(costMatrix, distMats{j}, lambda, addWeights, maximize,...
                  inputFileNameCostMat, outputFileNameAssign, 1, execPath);
    sec         = etime(clock,t0);
    if(sec <= 60)
    ...
    maximize    = 0;
    t0          = clock; 
    coeffOrder2 = linearAssignment(costMatrix, distMats{j}, lambda, addWeights, maximize,...
                  inputFileNameCostMat, outputFileNameAssign, 1, execPath);
    sec         = etime(clock,t0);
    if(sec <= 60)
