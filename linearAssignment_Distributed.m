...
%--------------------------------------------------------------------------
function assignments = linearAssignment_Distributed(costMatrix, weights, lambda,...
                                        addWeights, maximize, ...
...
%     [status shellText] = dos(['.\lap_binInFile\Release\linAssignBinFile.exe .\' inputFileName ' .\' outputFileName]);
    [status shellText] = dos([execLoc, 'linAssignBinFile.exe '...
                        inputFileName ' ' outputFileName]);
else
    [status shellText] = unix(['./lap_binInFileUnix/linAssignBinFile /var/tmp/apeter/' inputFileName ' ./' outputFileName]);
end
